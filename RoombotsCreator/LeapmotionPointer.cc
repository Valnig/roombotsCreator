...
void LeapmotionPointer::update(bool mode)
{
	...
	if (d_init )
	{
		...
		//handling the assignedRoomBot
		if (d_p_roombot != NULL)
		{
			//If the Leapmotion Pointer is linked to a RoomBot and the user's hand is still pinching, we move this RoomBot
			if (Pinching())
			{
				d_p_roombot->Drag(d_position);
			}
			else//Otherwise it means that the user's hand is open again and we can drop the RoomBot
			{
				std::cout << "pinching strength :" << d_controller.frame().hands().rightmost().pinchStrength() << std::endl;
				
				d_p_roombot->Drop();

				//the GUI is notified that a RoomBot has been dropped so it can pop a new one
				d_p_gui->DroppedRoomBot();
				d_p_roombot = NULL;
			}
		}
	}
...
void LeapmotionPointer::AssignRoomBot(RoomBot* p_RoomBot)
{
	d_p_roombot = p_RoomBot;
}

RoomBot* LeapmotionPointer::AssignedRoomBot() const
{
	return d_p_roombot;
}
